...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#0D0B21] to-black">
      {/* Grid Background */}
      <div 
        ref={gridRef}
      ...
      <div 
        className="absolute inset-0 z-0"
        style={{
          background: 'radial-gradient(circle at center, rgba(94,23,235,0.15) 0%, transparent 70%)'
        }}
      />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-screen flex flex-col justify-center">
        ...
        >
          ...
          >
            {/* Glow Effect */}
            <div className="absolute inset-0 rounded-full blur-xl bg-gradient-to-r from-[#5E17EB] to-[#3A86FF] opacity-40 group-hover:opacity-60 transition-opacity duration-300" />
            
            <button className="relative px-8 py-4 text-lg font-medium rounded-full bg-white text-[#0D0B21] hover:bg-gradient-to-r hover:from-[#5E17EB] hover:to-[#3A86FF] hover:text-white border border-transparent hover:border-white/20 transform hover:-translate-y-1 transition-all duration-300 shadow-lg hover:shadow-xl">
              Contact Us
            </button>
          </motion.div>
        </motion.div>
